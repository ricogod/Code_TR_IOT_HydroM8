...
class homePageState extends State<homePage> {
  ...
  final String timeToString = DateFormat('HH:mm:ss').format(DateTime.now());
  final String timeToInt= DateFormat('HHmmss').format(DateTime.now());
  ...
  void initState() {
    ...
    timeRef= FirebaseDatabase.instance.reference().child('test/timeDisplay');
   ...
   booleanRef.onValue.listen((event) {
    ...
    if (booleanValue) {
        FirebaseFirestore.instance
            .collection('history') // Replace with your collection name
            .add({
              'time': timeDisplay,
              'timeSort': timeSort,
              'date': date,
    ...
    timeRef.onValue.listen((event) {
      if (event.snapshot.value != null) {
        setState(() {
          timeAutoWater = event.snapshot.value.toString();
        });
      }
    });
  ...
  void _startClock() {
    _timer = Timer.periodic(const Duration(seconds: 1), (timer) {
      ...
      setState(() {
        ...
        timeDisplay = DateFormat('HH:mm').format(DateTime.now());
        timeSort= DateFormat('yyyyDDDHHmmss').format(DateTime.now());
        date= DateFormat('E, dd MMM yyyy').format(DateTime.now());
        DateTime parsedDate=DateTime.parse('2023-06-17'' ''10:46:00');
        String timeAutoWaterDisplay=DateFormat('HH:mm').format(parsedDate);
      });
